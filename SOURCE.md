...
The four additional sketches are placed in separate directories with matching sketch filenames to avoid combining their independent `setup()` and `loop()` functions. Documentation and the shared lab photo gallery are added for context. Five photographs disclose AI background/privacy retouching; the root-inspection image is an ordinary crop of the original. Photographs do not validate the archived code.
